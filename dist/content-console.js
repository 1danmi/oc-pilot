(() => {
  ...
  let FEATURES = {
    ...
    podActions:   true,   // Terminal / Logs / Events buttons in pod list
  ...
  function showConsoleToast(text, type = 'success') {
    ...
    host.setAttribute('style', [
      'all:initial',
      'position:fixed',
      // Error toasts go bottom-right so they never cover the masthead action
      // buttons (Copy Login, help icon, add, user menu) in the top-right corner.
      type === 'error' ? 'bottom:18px' : 'top:16px',
      'right:18px',
      'z-index:2147483647',
  ...
  const POD_ACTIONS = [
    { label: 'Terminal', path: '/terminal', color: '#10b981' },
    { label: 'Logs',     path: '/logs',     color: '#3b82f6' },
    { label: 'Events',   path: '/events',   color: '#a855f7' },
  ];
  ...
  function buildPodActionGroup(podPath) {
    ...
    POD_ACTIONS.forEach(({ label, path, color }) => {
  ...
  // Deferred, deduplicated resize notification for ReactVirtualized.
  // Using rAF means the DOM has fully settled before RV recalculates
  // _positionFromTop; the flag prevents double-fires when multiple callers
  // run within the same animation frame.
