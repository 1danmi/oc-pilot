...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  // Opens a background tab at the OAuth token-request URL with ?oc-pilot-silent=1.
  // content.js runs there: clicks "Display Token", reads the command from the
  // display page, and sends loginCommandReady back instead of writing to clipboard
  // (background tabs don't have user activation for clipboard writes).
  // A 20-second timeout ensures we always show a toast even if the flow fails.
  if (msg && msg.type === "copyLoginCommand" && msg.tokenRequestUrl) {
    ...
    chrome.tabs.create({ url: msg.tokenRequestUrl, active: false }, (tab) => {
      if (!tab || tab.id == null) {
        console.warn("[oc-pilot:bg] failed to create silent tab for:", msg.tokenRequestUrl);
        if (sourceTabId != null) {
          chrome.tabs.sendMessage(sourceTabId, {
            type: "ocPilotToast",
            text: "Could not open token-fetch tab",
            style: "error",
          }).catch(() => {});
        }
        return;
      ...
      console.log("[oc-pilot:bg] silent tab created — id:", silentTabId, "url:", tab.url || msg.tokenRequestUrl);
      ...
      const timer = setTimeout(() => {
        ...
        console.warn("[oc-pilot:bg] silent tab timed out — tabId:", silentTabId);
        silentTabSource.delete(silentTabId);
        chrome.tabs.remove(silentTabId).catch(() => {});
        if (sourceTabId != null) {
          chrome.tabs.sendMessage(sourceTabId, {
            type: "ocPilotToast",
      ...
      silentTabSource.set(silentTabId, { sourceTabId, timer });
    });
  ...
  if (msg && msg.type === "loginCommandFailed") {
    ...
    if (silentTabId != null) chrome.tabs.remove(silentTabId).catch(() => {});
